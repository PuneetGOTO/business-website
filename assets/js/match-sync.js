...
function syncMatchesToFrontend() {
    ...
    if (!matchesData.matches && (matchesData.match1Teams || matchesData.match2Teams || matchesData.match3Teams)) {
        ...
        if (matchesData.match1Teams) {
            matchesData.matches.push({
                ...
                team1Logo: matchesData.match1Team1Logo || 'assets/picture/upcoming_matches_1.png',
                team2Logo: matchesData.match1Team2Logo || 'assets/picture/upcoming_matches_2.png'
            });
        }
        ...
        if (matchesData.match2Teams) {
            matchesData.matches.push({
                ...
                team1Logo: matchesData.match2Team1Logo || 'assets/picture/upcoming_matches_1.png',
                team2Logo: matchesData.match2Team2Logo || 'assets/picture/upcoming_matches_2.png'
            });
        }
        ...
        if (matchesData.match3Teams) {
            matchesData.matches.push({
                ...
                team1Logo: matchesData.match3Team1Logo || 'assets/picture/upcoming_matches_1.png',
                team2Logo: matchesData.match3Team2Logo || 'assets/picture/upcoming_matches_2.png'
            });
        }
    ...
    if (matchCards.length === 0) {
        console.log('立即未找到元素，将在1秒后重试...');
        setTimeout(function() {
            matchCards = document.querySelectorAll('.upcoming_matches_content');
            console.log('1秒后重试，找到元素数量:', matchCards.length);
            
            if (matchCards.length > 0) {
                console.log('延迟后找到了比赛卡片，进行更新');
    ...
    function continueMatchSearch() {
        ...
        // 查找包含"VS"的元素
        if (matchCards.length === 0) {
            const vsElements = document.querySelectorAll('span:contains("VS"), span:contains("vs")');
        ...
        if (matchCards.length === 0) {
            ...
            // 尝试创建比赛元素
            const upcomingSection = document.querySelector('section:contains("比赛")') || 
                                   document.querySelector('section:contains("Upcoming")') || 
                                   document.querySelector('section:contains("match")');
            
            if (upcomingSection) {
                console.log('找到比赛section，尝试创建比赛元素');
                createMatchCards(upcomingSection, matchesData);
            } else {
                // 最后尝试查找所有section，查看哪个可能包含比赛
                console.log('尝试检查所有section寻找可能的比赛区域');
                const sections = document.querySelectorAll('section');
                for (let i = 0; i < sections.length; i++) {
                    const sectionText = sections[i].textContent.toLowerCase();
                    if (sectionText.includes('vs') || sectionText.includes('match') || 
                        sectionText.includes('tournament') || sectionText.includes('比赛')) {
                        console.log(`找到可能的比赛section[${i}]`);
                        createMatchCards(sections[i], matchesData);
                        break;
                    }
                }
            }
    ...
    function createMatchCards(container, data) {
        ...
        data.matches.forEach((match, index) => {
            ...
            // 确保所有图片路径都有默认值，防止404
            const team1Logo = match.team1Logo || 'assets/picture/upcoming_matches_1.png';
            const team2Logo = match.team2Logo || 'assets/picture/upcoming_matches_2.png';
    ...
    function updateMatchCard(card, data) {
        try {
            ...
            // 确保所有图片路径都有默认值，防止404
            const team1LogoSrc = data.team1Logo || 'assets/picture/upcoming_matches_1.png';
            const team2LogoSrc = data.team2Logo || 'assets/picture/upcoming_matches_2.png';
